...
import static org.neomatrix369.apiworld.util.UtilityFunctions.isAValidJSONText;
import static org.neomatrix369.apiworld.util.UtilityFunctions.isAValidXML;
...
public class APIReaderBehaviours {
	...
	String url = String.format("http://www.muzu.tv/api/browse?muzuid=%s&af=a&g=pop&format=%s", apiKey, JSON);
    ...
    public void should_Fetch_Data_As_XML_When_API_URL_Is_Passed_In() throws FileNotFoundException, IOException,
	    FinalURLNotGeneratedException {
	String apiKey = readPropertyFrom("resources/muzu_settings.properties", "APIKey");
	String url = String.format("http://www.muzu.tv/api/browse?muzuid=%s&af=a&g=pop&format=%s", apiKey, XML);
	APIReader apiReader = new APIReader(url);
	apiReader.executeURL();
	String result = apiReader.getFetchedResults();
